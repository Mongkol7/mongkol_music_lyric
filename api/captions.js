export default async function handler(req, res) {
  ...
  function isNoiseCaption(text) {
    ...
    if (t.includes('requested language') && t.includes('not available')) {
      return true;
    }
    if (t.includes('auto-generated by youtube')) return true;
    if (t.includes('subtitles/cc') || t.includes('subtitles')) return true;
    if (/^\[.*\]$/.test(text) && (t.includes('language') || t.includes('caption'))) {
      return true;
    }
    return false;
  }
  ...
  function cleanItems(items) {
    ...
    return items.filter((seg) => {
      const text =
        (seg && (seg.text || seg.caption || seg.content || seg.line || '')) || '';
      return text && !isNoiseCaption(text.trim());
    });
  }
  ...
  try {
    ...
    res.status(200).json({ items: cleanItems(items) });
  } catch (err) {
